...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h3 class="mb-0 text-capitalize font-weight-bold "> Albums ({{ $albums->total() }})</h3>
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($albums as $key => $alb)
                            <tr>
                                ...
                                <td>
                                    <a class="fancybox" data-fancybox="demo" href="{{ $alb->image }}">
                                        <img src="{{ $alb->image }}" alt="{{ $alb->name }}" width="120px"
                                            height="60px">
                                    </a>
                                </td>
                                ...
                                <td>
                                    ...
                                    <!-- Delete Album -->
                                    <form class="delete-form" action="{{ route('album.destroy', $alb->id) }}"
                                        method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button class="btn btn-sm btn-danger delete_albums mr-2 text-lg" data-type="confirm"
                                            type="submit" title="Delete" style="font-size:15px">
