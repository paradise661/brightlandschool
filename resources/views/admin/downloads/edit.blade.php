    ...
    <div class="content">
        <div class="card container-fluid mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Edit Downloads - {{ $download->name }}</h5>
            ...
            <div class="card-body p-0">
                <form class="row" method="POST" action="{{ route('downloads.update', $download->id) }}"
                            ...
                            <div class="form-group mb-3 d-flex align-items-center">
                                ...
                                <select class="form-select ms-5" id="status" name="status">
                                    <option class="p-3" value="0" @if ($download->status == 0) selected @endif>
                                        Draft</option>
                                    <option class="p-3" value="1" @if ($download->status == 1) selected @endif>
                                        Publish</option>
                                </select>
                            </div>
                                ...
                                <select class="form-select ms-5" name="download_categories_id" required>
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}"
                                            {{ old('download_categories_id', $download->download_categories_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
